...
import { formatGarbledReport } from "./utils/garbled-detector";
...
export interface MyPluginSettings {
    ...
    // learn panel
    learn_panel_font_size: string;
    auto_lemmatize: boolean;
...
export class SettingTab extends PluginSettingTab {
    ...
    display() {
        ...
        this.learnPanelSettings(containerEl);
    ...
    backendSettings(containerEl: HTMLElement) {
        containerEl.createEl("h3", { text: t("Backend Server") });
    ...
    querySettings(containerEl: HTMLElement) {
        ...
        new Setting(containerEl)
            ...
            .addButton(button => button
                .setButtonText(this.plugin.settings.popup_search ? t("Disable") : t("Enable"))
                .onClick(async () => {
                    this.plugin.settings.popup_search = !this.plugin.settings.popup_search;
                    this.plugin.store.popupSearch = this.plugin.settings.popup_search;
                    await this.plugin.saveSettings();
                    new Notice(this.plugin.settings.popup_search ? t("Popup search enabled") : t("Popup search disabled"));
                    this.display();
                })
            );
    ...
    indexedDBSettings(containerEl: HTMLElement) {
        ...
        // 诊断和修复数据库
        new Setting(containerEl)
            .setName(t("Diagnose & Fix"))
            .setDesc(t("Automatically check and fix database issues"))
            .addButton(button => button
                .setButtonText(t("Fix"))
                .onClick(async () => {
        ...
        // 检测乱码数据
        new Setting(containerEl)
            .setName("检测乱码数据" as any)
            .setDesc("查找表达式错乱的单词（如包含词性标记、释义等）" as any)
            .addButton(button => button
                .setButtonText("🔍 检测" as any)
                .onClick(async () => {
                    button.setDisabled(true);
                    button.setButtonText("检测中..." as any);
                    try {
                        const result = await this.plugin.detectGarbledWords();
                        if (result.garbled.length === 0) {
                            new Notice("✅ 未发现乱码数据");
                        } else {
                            const report = formatGarbledReport(result);
                            console.log("[LanguageLearner] Garbled Detection:\n" + report);
                            new Notice(`⚠️ 发现 ${result.garbled.length} 条乱码数据（详见控制台）`);
                        }
                    } catch (error) {
                        console.error(error);
                        new Notice("❌ 检测失败: " + error.message);
                    } finally {
                        button.setDisabled(false);
                        button.setButtonText("🔍 检测" as any);
                    }
                })
            )
            .addButton(button => button
                .setButtonText("🧹 清理" as any)
                .setWarning()
                .onClick(async () => {
                    const { WarningModal } = await import("./modals");
                    let modal = new WarningModal(
                        this.app,
                        `确定要删除所有乱码数据吗？此操作不可撤销！`,
                        async () => {
                            button.setDisabled(true);
                            button.setButtonText("清理中..." as any);
                            try {
                                const deleted = await this.plugin.cleanGarbledWords();
                                if (deleted > 0) {
                                    new Notice(`✅ 已删除 ${deleted} 条乱码数据`);
                                }
                            } catch (error) {
                                console.error(error);
                                ...
                                button.setDisabled(false);
                                button.setButtonText("🧹 清理" as any);
                            }
                        }
                    );
                    modal.open();
    ...
    textDBSettings(containerEl: HTMLElement) {
        ...
        new Setting(containerEl)
            .setName(t("Auto fill meanings"))
            .setDesc(t("Auto fill Chinese meaning from dictionary when adding new words"))
            .addToggle(toggle => toggle
                .setValue(this.plugin.settings.auto_fill_meanings)
                .onChange(async (value) => {
                    this.plugin.settings.auto_fill_meanings = value;
                    await this.plugin.saveSettings();
                })
            );
        ...
        new Setting(containerEl)
            .setName(t("Import Word Database"))
            .setDesc(t("Import words from wordDB to IndexedDB"))
            .addButton((btn) =>
                btn
                    .setButtonText(t("Import"))
                    .onClick(async () => {
                        await this.plugin.importWordDB();
                    })
            );
    }
    ...
    learnPanelSettings(containerEl: HTMLElement) {
        containerEl.createEl("h3", { text: t("Learn Panel") });

        new Setting(containerEl)
            .setName(t("Font Size"))
            .setDesc(t("Like 0.5em or 12px"))
            .addText(text => text
                .setValue(this.plugin.settings.learn_panel_font_size)
                .onChange(debounce(async (value) => {
                    this.plugin.settings.learn_panel_font_size = value;
                    this.plugin.store.learnPanelFontSize = value;
                    await this.plugin.saveSettings();
                }, 500))
            );

        new Setting(containerEl)
            .setName(t("Auto Lemmatize") as any)
            .setDesc("在学习面板输入时自动还原词形（如 running → run）。暂未完善此功能，默认关闭" as any)
            .addToggle(toggle => toggle
                .setValue(this.plugin.settings.auto_lemmatize)
                .onChange(async (value) => {
                    this.plugin.settings.auto_lemmatize = value;
                    await this.plugin.saveSettings();
                })
            );
        ...
        new Setting(containerEl)
            ...
            .addToggle(toggle => toggle
                ...
                .onChange(async (value) => {
                    this.plugin.settings.auto_mark_lemma_variants = value;
                    await this.plugin.saveSettings();
                })
            );
        ...
        new Setting(containerEl)
            ...
            .addToggle(toggle => toggle
                ...
                .onChange(async (value) => {
                    this.plugin.settings.auto_generate_variants = value;
                    await this.plugin.saveSettings();
                })
            );

        new Setting(containerEl)
            .setName("词形变体数据库路径" as any)
            .setDesc("填写 vault 下的 md 文件名作为词形变体数据库，如 word-variants.md。变体将在提交单词时自动写入此文件" as any)
            .addText((text) =>
                text
                    .setValue(this.plugin.settings.variant_note_path)
                    .setPlaceholder("word-variants.md")
                    .onChange(async (path) => {
    ...
    reviewSettings(containerEl: HTMLElement) {
        ...
        new Setting(containerEl)
            ...
            .setDesc(t("Sync all words to review flashcards"))
            .addButton(button => button
                ...
                .onClick(async () => {
                    ...
                    try {
                        ...
                        new Notice(t("Review database refreshed successfully"));
                    } catch (error) {
                        console.error("Failed to refresh review database:", error);
                        new Notice(t("Failed to refresh review database"));
                    } finally {
                        button.setDisabled(false);
